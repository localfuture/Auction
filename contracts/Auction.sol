...
contract Auction {
	...
	function createAuction(uint256 itemNumber,uint256 startingPrice,uint256 duration) public {
        require(owner == msg.sender, "Owner must be");
        ...
        auctions[itemNumber].highestBidder = address(0);
        auctions[itemNumber].bid = 0;
	...
	function bid(uint256 itemNumber, uint256 bidAmount) public payable { 
        ...
        require(msg.value > auction.bid, "Bid must be higher than current highest bid");

        if (auction.highestBidder != address(0)) {
            payable(auction.highestBidder).transfer(bidAmount); // Refund previous highest bidder
        }
	...
	function cancelAuction(uint256 itemNumber) public { 
        ...
        if (auction.highestBidder != address(0)) {
            payable(auction.highestBidder).transfer(auction.bid); // Refund previous highest bidder
        }
    }
